...
def StopButtonChecking():
    ...
    while(isEnd==False):
        ...
        if isStopBtnPressed == 1:
            isEnd = True
            print("PickUp, Stop, Error")

            now_ai_name = ai_name_prefix + "PickUp," + item_name + ",Error"
            print(f"#{now_ai_name}")
            me.stop(now_ai_name)
            while(1):
                misc = me.get_misc()
...
now_ai_name = ai_name_prefix + "PickUp," + item_name + ",Start"
print(f"#{now_ai_name}")
me.stop(now_ai_name)
while(1):
    misc = me.get_misc()
...
SetModbus(ip_addr="192.168.88.7", port=502, slave_id=1, reg_or_coil="HOLD_REG", \
          st_addr=7701, quantity=1, output_value=0x5130, name="taskindex0")

WaitModbusSingleEqual(ip_addr="192.168.88.7",port=502, slave_id=1, reg_or_coil="INPUT_REG", \
                      st_addr=7701, quantity=1, cmp_value=0x5130, name="taskindex0", waittimeout=-1)

# ====================================

now_ai_name = ai_name_prefix + "PickUp," + item_name + ",Running"
print(f"#{now_ai_name}")
me.stop(now_ai_name)
while(1):
    misc = me.get_misc()
...
if (isCobotPause==1):
    print("PickUp, Final Pause, Error")
...
if (isCobotError==0) and (isCobotPause==0) and (isCobotEstop==0):
    ...
    now_ai_name = ai_name_prefix + "PickUp," + item_name + ",End"
    print(f"#{now_ai_name}")
    me.stop(now_ai_name)
    while(1):
        misc = me.get_misc()
