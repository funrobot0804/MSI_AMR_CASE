...
while(1):
    try:
        ...
        if is_already_init == 0:
            aco.pre_calculate_V2()
            is_already_init = 1

        people_inside_val = aco.calculate_V2()

        print(f"people_inside_val = {people_inside_val}")

        if people_inside_val == 3:
            if is_people_pause_now == 1:
                print("People_Leave_Pause_Area")
                aco.SetPauseState(0)
                ret = the_modbus_master.execute(1, md.WRITE_SINGLE_COIL, \
                                                starting_address=7104, \
                                                quantity_of_x=1, \
                                                output_value=1)

                is_people_pause_now = 0

                sound_filepath = "cobot_resume.mp3"
                if os.path.isfile("/data/wav/cobot_resume.mp3") == True:
                    sound_filepath = "/data/wav/cobot_resume.mp3"

                playsound_thread = threading.Thread(target=PlaySound, args=[sound_filepath])
                playsound_thread.start()

                aco.SaveDebugImage(f"/data/dump/AMR_CheckOccur2_{counter}.png")
                counter += 1
                # os._exit(1)

        if people_inside_val == 1:
            if is_people_pause_now == 0:
                print("People_In_Pause_Area")
                aco.SetPauseState(1)
                ret = the_modbus_master.execute(1, md.WRITE_SINGLE_COIL, \
                                                starting_address=7104, \
                                                quantity_of_x=1, \
                ...
                is_people_pause_now = 1

                sound_filepath = "cobot_pause.mp3"
                if os.path.isfile("/data/wav/cobot_pause.mp3") == True:
                    sound_filepath = "/data/wav/cobot_pause.mp3"

                playsound_thread = threading.Thread(target=PlaySound, args=[sound_filepath])
                playsound_thread.start()


        if people_inside_val == 2:
            print("People_In_Danger_Area")
            ret = the_modbus_master.execute(1, md.WRITE_SINGLE_COIL, \
                                            starting_address=7105, \
                                            quantity_of_x=1, \
                                            output_value=1)

            sound_filepath = "cobot_stop.mp3"
            if os.path.isfile("/data/wav/cobot_stop.mp3") == True:
                sound_filepath = "/data/wav/cobot_stop.mp3"

            playsound_thread = threading.Thread(target=PlaySound, args=[sound_filepath])
            playsound_thread.start()

            me.stop("error,TM_StartProj_PeopleInDangerArea")
            os._exit(1)
