...
class AMR_CheckOccur():
    def __init__(self, amr_ip="127.0.0.1"):
        ...
        self.resume_rect_vertices = None
        self.resume_rect_vertices_grid = None
    ...
    def enable_detect_point(self, ai_name="AMR_CheckOccur, enable_detect_point"):
        ...
        self.me.scan(name=ai_name, opt="LASER", alarm_enable=False)
    ...
    def get_detect_point(self):
        ...
        while(1):
            ...
            if the_plan_ring is not None:
                if the_plan_ring.shape[0] > 0:
                    break
        ...
        print("self.the_plan_ring =", self.the_plan_ring)
    ...
    def pre_calculate_V2(self):
        ...
        self_pos_0_resume = [ self_pos_0[0] + (self.pause_range+0.2)*self_pos_0_uvec[0], \
                              self_pos_0[1] + (self.pause_range+0.2)*self_pos_0_uvec[1] ]
        ...
        self_pos_1_resume = [ self_pos_1[0] + (self.pause_range+0.2)*self_pos_1_uvec[0], \
                              self_pos_1[1] + (self.pause_range+0.2)*self_pos_1_uvec[1] ]
        ...
        self_pos_2_resume = [ self_pos_2[0] + (self.pause_range+0.2)*self_pos_2_uvec[0], \
                              self_pos_2[1] + (self.pause_range+0.2)*self_pos_2_uvec[1] ]
        ...
        self_pos_3_resume = [ self_pos_3[0] + (self.pause_range+0.2)*self_pos_3_uvec[0], \
                              self_pos_3[1] + (self.pause_range+0.2)*self_pos_3_uvec[1] ]
        ...
        self.resume_rect_vertices = [  tuple(self_pos_0_resume), \
                                       tuple(self_pos_1_resume), \
                                       tuple(self_pos_2_resume), \
                                       tuple(self_pos_3_resume)
                                    ]
        ...
        self_pos_0_resume_grid = [0, 0]
        self_pos_0_resume_grid[0], self_pos_0_resume_grid[1] = self.Pos2Grid(self_pos_0_resume[0], \
                                                                             self_pos_0_resume[1])

        self_pos_1_resume_grid = [0, 0]
        self_pos_1_resume_grid[0], self_pos_1_resume_grid[1] = self.Pos2Grid(self_pos_1_resume[0], \
                                                                             self_pos_1_resume[1])

        self_pos_2_resume_grid = [0, 0]
        self_pos_2_resume_grid[0], self_pos_2_resume_grid[1] = self.Pos2Grid(self_pos_2_resume[0], \
                                                                             self_pos_2_resume[1])

        self_pos_3_resume_grid = [0, 0]
        self_pos_3_resume_grid[0], self_pos_3_resume_grid[1] = self.Pos2Grid(self_pos_3_resume[0], \
                                                                             self_pos_3_resume[1])

        self.resume_rect_vertices_grid = [  tuple(self_pos_0_resume_grid), \
                                            tuple(self_pos_1_resume_grid), \
                                            tuple(self_pos_2_resume_grid), \
                                            tuple(self_pos_3_resume_grid)
                                         ]
    ...
    def calculate_V2(self):
        ...
        for i in range(16, 28):
            ...
            abs_p_info = math.fabs(p_info)
            #print(f"i={i}, int(abs_p_info)={int(abs_p_info)}")
            sensor_status = int(int(abs_p_info) / 1000) % 100
            #print(f"i={i}, A. sensor_status={sensor_status}")
            sensor_status = int(math.floor(sensor_status))
            #print(f"i={i}, B. sensor_status={sensor_status}")
            ...
            rect_vertices = self.resume_rect_vertices
            is_in_resume_range = self.is_inside_rectangle(rect_vertices, test_point)

            if sensor_status!=1:
                print(f"i={i}, sensor_status={sensor_status}, >, <, !=1")
        ...
        if ENABLE_DEBUG_DRAWING == 1:
            ...
            for i, p in enumerate(self.resume_rect_vertices):
                if (i > 0):
                    x0 = self.resume_rect_vertices_grid[i - 1][0]
                    x1 = self.resume_rect_vertices_grid[i][0]
                    y0 = self.resume_rect_vertices_grid[i - 1][1]
                    y1 = self.resume_rect_vertices_grid[i][1]
                ...
                if i == 3:
                    x0 = self.resume_rect_vertices_grid[0][0]
                    x1 = self.resume_rect_vertices_grid[i][0]
                    y0 = self.resume_rect_vertices_grid[0][1]
                    y1 = self.resume_rect_vertices_grid[i][1]
            ...
            for g in self.detect_point_list:
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 255
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 0
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 0

            for g in self.laser_set_point_list:
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 184
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 134
                color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 11
        ...
        print(f"resume_count = {resume_count}, valid_data_count = {valid_data_count}")
        if resume_count >= valid_data_count:
            if calc_result != 2:
                calc_result = 3
    ...
    def SaveDebugImage(self, filepath="AMR_CheckOccur2.png"):
        color_img_array = np.arange(0, 8192 * 8192 * 3, 1, np.uint8)
        color_img_array = color_img_array.reshape(8192, 8192, 3)
        color_img_array.fill(255)
        ...
        for g in self.detect_point_list:
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 255
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 1] = 0
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 2] = 0

        for g in self.laser_set_point_list:
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 0] = 184
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 1] = 134
            color_img_array[g[1]-1:g[1]+2, g[0]-1:g[0]+2, 2] = 11

        gui_color_img_array = np.flipud(color_img_array)
        img_file_data = im.fromarray(gui_color_img_array)
        img_file_data.save(filepath)
